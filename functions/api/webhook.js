import {
  ...
  verifyFedaPaySignature, addResumePurchase,
...
export async function onRequestPost(context) {
  ...
  const valid = await verifyFedaPaySignature(FEDAPAY_WEBHOOK_SECRET_HARDCODED, rawBody, sig);
  if (!valid) {
    ...
    if (fedaEvent.name === 'transaction.approved') {
      ...
      if (already && already.type === 'resume') {
        if (already.status !== 'paid') {
          await addResumePurchase(kv, already.phone, already.classe, already.matiere, tx.id);
          await kvSetJSON(kv, txKey, { ...already, status: 'paid', paidAt: Date.now() });
