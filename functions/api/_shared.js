...
/* Stockés comme une liste sous une seule clé KV 'resume:list'.
   Chaque résumé : { id, matiere, classe, titre, texte, images:[urls], createdAt, updatedAt }
   Les images (photos illustrant le résumé) sont stockées dans le bucket R2 FPB_IMAGES
   sous le préfixe "resumes/" et référencées via des URLs "/api/get-image?key=...". */
...
/* ---------- Matières masquées (par classe) pour les résumés ---------- */
/* Certaines matières officielles proposées par classe n'ont pas besoin de résumé.
   L'admin peut les masquer : elles disparaissent alors de la liste déroulante
   "matière" lors de l'ajout d'un résumé (admin) et de la liste de matières
   affichée aux enseignants dans l'appli. Stocké comme une liste de chaînes
   "classe::matiere" sous une seule clé KV 'resume:hidden-matieres'. */
export const RESUME_HIDDEN_MATIERES_KEY = 'resume:hidden-matieres';

export async function getHiddenMatieres(kv) {
  const raw = await kv.get(RESUME_HIDDEN_MATIERES_KEY);
  return raw ? JSON.parse(raw) : [];
}

export async function saveHiddenMatieres(kv, list) {
  await kv.put(RESUME_HIDDEN_MATIERES_KEY, JSON.stringify(list));
}
...
export const RESUME_CLASSE_BUNDLE_PRICE = 2000; // FCFA, toutes les matières d'une classe
export const RESUME_ALL_BUNDLE_PRICE = 5000;    // FCFA, toutes les classes (CI à CM2)
