import { normalizePhone, json, kvKeyUser, kvGetJSON, getResumeList, getResumePurchases, getHiddenMatieres } from './_shared.js';
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const filtered = list
      .map(r => ({ id: r.id, matiere: r.matiere, classe: r.classe, unite: r.unite || '', sa: r.sa || '', sequence: r.sequence || '', dossier: r.dossier || '', titre: r.titre, texte: r.texte, images: r.images || [], updatedAt: r.updatedAt }))
    ...
    const hiddenMatieres = await getHiddenMatieres(kv);

    return json({ ok: true, resumes: filtered, purchases, hiddenMatieres });
  } catch (err) {
